...
class _GroupFinderDetailScreenState
  ...
  _setButton(Group groupDetail) {
    if (groupDetail.memberStatus == "in") {
      return Center(child: Text("가입된 그룹 입니다"));
    } else if (groupDetail.memberStatus == "mid") {
      return Center(child: Text("가입신청을 보냈습니다"));
    } else {
      return Align(
        ...
        child: OutlinedButton(
          onPressed: () async {
            var url =
                Uri.http(baseUri, '/group/sendRequest/${groupDetail.groupId}');
            var response = await http
                .post(url, headers: {'authorization': 'Bearer $jwtToken'});
            if (response.statusCode == 200) {
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              ...
                              Text(
                                  '(${groupDetail.nowMember} / ${groupDetail.maxMember})')
                            ],
                          ),
                          SizedBox(
                            height: 10,
